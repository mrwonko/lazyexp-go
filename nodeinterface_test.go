...
func TestNodeDependencies(t *testing.T) {
	...
	for _, testIO := range []struct {
		...
		{"CancelOnError: single success", lazyexp.Dependencies{lazyexp.DiscardOnError(nilNode)}, []error{nil}, fetchErr},
		{"CancelOnError: multi success", lazyexp.Dependencies{lazyexp.DiscardOnError(nilNode), lazyexp.DiscardOnError(nilNode)}, []error{nil, nil}, fetchErr},
		{"CancelOnError: single failure", lazyexp.Dependencies{lazyexp.DiscardOnError(errNode1)}, []error{err1}, fetchErr},
		{"CancelOnError: cancelling failure", lazyexp.Dependencies{lazyexp.DiscardOnError(errNode1), lazyexp.ContinueOnError(noreturnNode)}, []error{err1, lazyexp.Discarded}, fetchErr},
